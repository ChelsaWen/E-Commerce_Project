## Explore dataset, chekcking null or duplicate values 
...
LIMIT 30;
## "canceled" and "unavailable" are statuses that do not involve actual transactions, exclude them from valid orders
...
-- 4.1 Customer geolocation distribution
-- 4.1.1 Top 10 states 
...
-- 4.2 Customer Payment types distrbution 
...
-- 4.3.1 Data preparation for RFM Analysis 
...
-- 4.3.2 Caculate the number of one-time purchasers
...
## Therefore, the analysis will not proceed with the calculation of CLV or Retention rate
 
-- 4.4 Customer Satisfaction 

-- 4.4.1 Calculate average rating score 
SELECT AVG(review_score) 
FROM Reviews;
## Average rating score: 4.0864

-- 4.4.2 Score distribution
...
-- 4.4.3 Create view for rating: order with score >= 4 is considered as satisfied, < 4 as dissatisfied
...
## Further analysis will be continue in NLP analysis in Jupyter Notebook

-- 5. Product Analysis --

-- 5.1 Total products sold, total unique products sold
SELECT COUNT(DISTINCT product_id) AS TotalUniqueProducts, 
		COUNT(order_item_id) AS TotalProducts 
        FROM Price;
## Total unique products sold: 32,951; Total products sold: 112,650

-- 5.2 Number of orders, Total sales, Average unit price for each product & its category
SELECT pr.product_id, product_category_name AS category, SUM(price) AS TotalSales, 
		COUNT(DISTINCT order_id) AS NumOfOrders, SUM(price)/COUNT(order_item_id) AS AverageUnitPrice
FROM Price pr
LEFT JOIN Products p
ON pr.product_id = p.product_id
GROUP BY pr.product_id, product_category_name
ORDER BY TotalSales DESC;

-- 5.4 Number of unique products in each category

-- 5.5 Top 3 best selling category
...
-- 5.6 Top 3 categories and their Best 3 products
